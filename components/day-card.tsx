  ...
  import { isToday, requestNotificationPermission, scheduleNotification, parseTime } from "@/lib/schedule-utils"
  ...
  import { IconBell, IconBellOff, IconBriefcase, IconClock, IconGripVertical, IconSettings2, IconBriefcaseOff } from "@tabler/icons-react"
  ...
  function SortableActivity({
    ...
    const [hasReminder, setHasReminder] = useState<boolean>(() => !!(activity as any).hasReminder)
    ...
    const handleReminderToggle = async () => {
      if (!hasReminder) {
        ...
        if (permitted) {
          ...
          scheduleNotification("Activity Reminder", `Time to: ${activityName}`, delayMs)
          onRemind()
        }
      } else {
        ...
        >
          <CardHeader className="space-y-2 pb-3">
            <div className="flex items-center justify-between">
              <CardTitle
                className={`text-lg font-medium ${language === "kh" ? "font-khmer" : ""} ${today ? "glow-text text-primary" : ""}`}
              >
                {dayName}
              </CardTitle>
